...
| 5b Thread home, streaming chat, and first-pass context | `red-maple` | `/app/rhizome` thread home/list, `/app/rhizome/:threadId`, composer, streaming messages, topbar thread switcher, read-only model display, session controls, first-pass interaction panel, resume stream actions, composer context tray/search/pinning | No-thread state can start a thread; existing threads are selectable; sending a message streams tokens and ends cleanly; pending interaction opens review panel; adding/removing context updates composer chips and backend |
| 5c Rich interaction/context entry polish | `silver-maple` | Compact inline interaction summaries, richer typed review cards, pending interaction queue affordances if the API exposes more than one pending item, context-aware entry modal, inline `plant:`/`task:` object tagging, agent-authored object links that open the right inspection panel | Inline interaction summaries render in chat; context-aware entry from detail pages can create a new contextual thread or add context to the active thread; object references can be discovered from the composer or Rhizome replies |
...
7. **Composer context basics:** Render pinned/attached context inside the composer, search across objects with unified and typed search, add context to the active thread, pass attached context when creating a blank thread, and remove context chips. Context-aware entry from other pages remains a 5c concern.
...
9. **5b test and live smoke pass:** Add focused component/page coverage for route loads, fallback `getThread`, history rendering, composer state, stream races/cancel/error, session controls, interaction review/resume, and context pin/remove, plus an opt-in live Cambium/Rhizome smoke for real streaming with the available providers.

**5b status:** Slices 1-8 are implemented and pushed on `red-maple`. `/app/rhizome` now has an API-backed workbench shell, recent-thread shortcuts in the blank state, open-on-demand thread navigator, selected-thread route shell, typed message history rendering, read-only themed model selector, dedicated session-context display/edit controls, a functional composer, first-pass interaction review panel with resume streaming, and composer-based context search/add/remove. First send from the blank state creates a thread through Cambium, navigates to the returned `/app/rhizome/:threadId`, streams Rhizome's response, and exposes a top-of-conversation retry banner for stream failures. Day separators render when message timestamps are present; Rhizome's current message endpoint only returns role/content/type, so existing history currently renders without date breaks. Remaining 5c work is richer inline interaction summaries, typed review card variants, queue affordances if the backend exposes multiple pending interactions, context-aware entry from object/detail pages, inline object tagging, and agent-authored object links that open the right inspection panel.
