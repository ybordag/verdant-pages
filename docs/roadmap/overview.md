...
| 5b Thread home, streaming chat, and first-pass context | `red-maple` | `/app/rhizome` thread home/list, `/app/rhizome/:threadId`, composer, streaming messages, topbar thread switcher, read-only model display, session controls, first-pass interaction panel, resume stream actions, message-context UI, pinned-thread context search/add/remove | No-thread state can start a thread; existing threads are selectable; sending a message streams tokens and ends cleanly; pending interaction opens review panel; pinned context updates chips and backend |
...
**5b status:** Slices 1-8 are implemented and pushed on `red-maple`. `/app/rhizome` now has an API-backed workbench shell, recent-thread shortcuts in the blank state, open-on-demand thread navigator, selected-thread route shell, typed message history rendering, read-only themed model selector, dedicated session-context display/edit controls, a functional composer, first-pass interaction review panel with resume streaming, local message-context UI, and fully wired pinned thread context search/add/remove. First send from the blank state creates a thread through Cambium, navigates to the returned `/app/rhizome/:threadId`, streams Rhizome's response, and exposes a top-of-conversation retry banner for stream failures. Day separators render when message timestamps are present; Rhizome's current message endpoint only returns role/content/type, so existing history currently renders without date breaks. Remaining 5c work is richer inline interaction summaries, typed review card variants, queue affordances if the backend exposes multiple pending interactions, context-aware entry from object/detail pages, backend-supported per-message context, inline object tagging, and agent-authored object links that open the right inspection panel.
