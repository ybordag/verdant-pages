...
## Vite + React 19 + TypeScript + React Router v6

| Layer | Choice | Rationale |
|---|---|---|
| Bundler | Vite | Sub-second HMR, TypeScript first-class, production build via Rollup. Simpler than Next.js for a pure SPA. |
| UI | React 19 | Current app dependency. Keeps the prototype port on modern React while preserving the same SPA architecture. |
...
| Routing | React Router v6 | Client-side SPA routing, nested layouts, protected routes, `useNavigate` for 401→login redirect. |
