...
**Password** — a "Change password" form: current password field, new password field, confirm field. Submit calls `POST /auth/password`. Inline success/error feedback.
...
**Provider picker** — segmented control: Gemini / OpenAI / Anthropic. Saves to `preferred_provider` via `PATCH /auth/profile`.
...
| `PATCH /auth/profile` | Update preferred_provider, preferred_model |
